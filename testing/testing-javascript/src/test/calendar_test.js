...
$(document).ready(function() {
	var cal1 = $("#dateRange1").continuousCalendar({
		...
		firstDate 		: null,
		lastDate 		: null,
		startField 		: $('input.startDate1', this),
		endField 		: $('input.endDate1', this),
		isPopup 		: false,
		selectToday 	: false,
		locale 			: 'EN',
	...
	$("#dateRange2").continuousCalendar({
		...
		firstDate 		: null,
		lastDate 		: null,
		startField 		: $('input.startDate2', this),
		endField 		: $('input.endDate2', this),
		isPopup 		: true,
		selectToday 	: false,
		locale 			: 'EN',
		disableWeekends : false,
	...
	$("#dateRange1").on('calendarChange', function(){
		var start = $('input.startDate1').val();
		var end = $('input.endDate1').val();
		...
		test("dateRange1 depth1", function(){
			equal(start, $('input.startDate1').val(), "start == input.startDate1");
			equal(end, $('input.endDate1').val(), "end == input.endDate1");
...
//					}
					...
					try {
						ff_chr_responseText = $.parseJSON(result.responseText);
					} catch (e) {
						$("#console2").append(e);
					}
					
					equal($(ff_chr_responseText).attr("depth1"), "menu", "text depth1 is \"menu\"");
					equal($(ff_chr_responseText).attr("depth2"), "list", "text depth2 is \"list\"");
					...
					calenderAlert(getDateString(start.split("/")) + " ~ " + getDateString(end.split("/")) + "<br/>" + "depth1 : " + $(ff_chr_responseText).attr("depth1") + ", depth2 : " + $(ff_chr_responseText).attr("depth2"));
				});
	...
	$("#dateRange2").on('calendarChange', function(){
		var start = $('input.startDate2').val();
		var end = $('input.endDate2').val();
		$('#console2').html(getDateString(start.split("/")) + " | " + getDateString(end.split("/")));
	});
});
