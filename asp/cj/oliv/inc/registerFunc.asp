<%
	'회원가입
	Function createImUserQuery(jsonData)
...
'		Dim Param(58)
	...
	sub checkUpdate(jsonData)
		...
		Glb.Dbc.Execute "up_NAdaptor_existUser", Param
		ERRSTAT	= Glb.Dbc.GetValue(Param, "@ERRSTAT")
		ERRMSG	= Glb.Dbc.GetValue(Param, "@ERRMSG")

'response.write  imData.Get("IPIN_CI") & "<BR>"
'response.write  imData.Get("MBR_NO") & "<BR>"
'response.write LCase(ERRSTAT)
'response.End()

		If LCase(ERRSTAT) = "true" Then		'해당 회원이 존재하므로 update로 보낸다.
			Server.Execute("update.asp")
			response.End
		Else								'약관철회 회원의 경우 인지 조회한다.
...
'''
'''		'IM_USER, VIEW_IM_USER 쿼리, redirect url
'''		Dim imData, sql, url
'''
'''		'회원정보 존재여부
'''		Dim existYn : existYn = "Y"
'''
'''		Dim objRs : Set objRs = Server.CreateObject("ADODB.RecordSet")
'''
'''		Set imData = JSON.parse(jsonData.Get("IM_USER"))
'''
'''		sql = "SELECT * FROM VIEW_IM_USER"
'''		sql = sql & " WHERE MBR_NO = '" & imData.Get("MBR_NO") & "'"
'''
